...
function VariantCard({ variant }: { variant: IProductVariant }) {
                ...
                <Flex h="100%" flexDirection="column" justifyContent="space-between">
                    <Center className="cardImgContainer" m={3} h="50%">
                        <NextLink href={`/product/${variant.parentId}/${variant.id}`}>
                            <a>
                                <Image src={variant.imageURL} display="block" alt={`Obrázek ${variant.title}`} fit="scale-down" roundedTop="lg" maxH={180} maxW={180} mt={5} />
                            </a>
                        </NextLink>
                    </Center>
